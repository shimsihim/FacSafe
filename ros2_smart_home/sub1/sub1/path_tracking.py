...
class followTheCarrot(Node):
    def __init__(self):
        ...
        # 로직 2. 파라미터 설정
        # Look Forward Distance"의 약어로, 로봇이 주어진 경로 상에서 전방을 얼마나 주시할 거리인지를 나타내는 변수
        self.lfd = 0.1
        self.min_lfd = 0.1
        self.max_lfd = 1.0

    def timer_callback(self):
        # 로봇의 위치, 경로, 상태 메시지가 모두 들어온 상태에서 동작
        if self.is_status and self.is_odom == True and self.is_path == True:
            if len(self.path_msg.poses) > 1:
                
                # 전방주시 포인트를 찾았는지 여부를 말하며, 아직 어디를 보는 지 설정 X므로 FALSE
                ...
                if self.is_look_forward_point:
                    ...
                    x = local_forward_point[0]
                    y = local_forward_point[1]
                    theta = atan2(y, x)
                    '''
                    
                    # 로봇좌표계에서 기준좌표계(Map)로 좌표변환을 하기위한 변환 행렬
                    trans_matrix = np.array(
                        [
                            [cos(self.robot_yaw), sin(self.robot_yaw), robot_pose_x],
                            [sin(self.robot_yaw), cos(self.robot_yaw), robot_pose_y],
                    ...
                    # det_trans_matrix는 trans_matrix 의 역행렬
                    det_trans_matrix = np.linalg.inv(trans_matrix)
    ...
    def odom_callback(self, msg):
        ...
        # 변수 언패킹(Unpacking)을 사용하는 코드
        # q.to_euler() 함수로부터 반환되는 값을 변수에 할당하는 데 사용
        # _는 일종의 더미 변수로 언패킹 과정에서 변수를 무시하고 싶을 때 _를 사용
